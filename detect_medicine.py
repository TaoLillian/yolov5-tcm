#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2021/6/11 18:31
# @Author  : Tianle Hu
# @Site    :
# @File    : detect_medicine.py
# @Software: PyCharm
import argparse
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                if names[int(c)] in ['aiye','chenpi','gouqizi','honghua','juemingzi','sangshen','shanzha','xiakucao','xixin', 'zhangshenwen','xuchonghai', 'hutianle']:
                    topic = 'goods_response'
                    message = names[int(c)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='http://172.20.10.10:8003/?action=stream', help='source')  # file/folder, 0 for webcam
    #parser.add_argument('--source', type=str, default='0', help='source')
    # 网络输入图片大小
    parser.add_argument('--img-size', type=int, default=480, help='inference size (pixels)')
    # 置信度阈值，检测到的对象属于特定类（艾叶，白矾，陈皮，枸杞子等）的概率
